 ...
 * - Shows a loading state while the SDK initializes
 * - Shows "Log in or sign up" when not authenticated (Google + Email OTP)
 * - Shows the user's address/email with a dropdown when authenticated
 */
...
export default function DynamicAuthButton() {
  ...
  if (loggedIn) {
    ...
    const initials = (
      userEmail ? userEmail.slice(0, 2) : primary.slice(addrStart, 2)
    ).toUpperCase();
    return (
