...
import { signInWithSocialRedirect, logout, sendEmailOTP, verifyOTP } from "@dynamic-labs-sdk/client";
...
function AddressRow({ label, addr, copied, onCopy }) {
  return (
...
export default function DynamicButton() {
  ...
  if (initStatus !== "finished") {
    return (
      <button
        disabled
        className="inline-flex items-center justify-center gap-2 px-4 py-2 border border-gray-300 rounded-md text-sm font-medium text-gray-400 bg-white opacity-70 cursor-not-allowed"
      >
  ...
  if (loggedIn) {
    const primary = evmWallet?.address ?? "";
    const userEmail = user?.email ?? "";
    const label = userEmail ? truncateEmail(userEmail) : truncate(primary);
    const addrStart = primary.startsWith("0x") ? 2 : 0;
    const initials = (userEmail ? userEmail.slice(0, 2) : primary.slice(addrStart, 2)).toUpperCase();
              ...
              {accounts.length === 0 ? (
                ...
                evmWallet && (
                  <AddressRow
                    ...
                    copied={copied === "evm"}
                    onCopy={() => copy(evmWallet.address, "evm")}
                  />
                )
              )}
            </div>
  ...
  return (
    <div className="relative" ref={ref}>
      <button
        ...
        className="cursor-pointer inline-flex items-center justify-center px-4 py-2 border border-transparent text-sm font-medium rounded-md text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-colors"
      >
        Log in or sign up
      </button>
